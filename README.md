...
В Mesh архитектуре каждый пользователь подключается напрямую к каждому другому пользователю в голосовом канале. Через сервер проходит только сигнализация (настройка соединения), сами аудиоданные идут напрямую между участниками.
Пользователь А (создатель канала)
│
├─────────────────────────────────────┐
│ │
▼ ▼
...
bash
npm start
Откройте браузер по адресу https://ваш-сервер:3000 (или http://localhost:3000 для локальной разработки)
...
Проверьте, что микрофон не отключён в системных настройках

Не устанавливается WebSocket соединение
Проверьте, запущен ли сервер

Убедитесь, что брандмауэр не блокирует порт

Для HTTPS убедитесь в правильности SSL сертификатов

Не слышно других пользователей
Проверьте, что другие пользователи находятся в том же голосовом канале

Убедитесь, что динамики не отключены

Проверьте консоль браузера на наличие ошибок WebRTC

Сообщения не появляются в реальном времени
Проверьте статус WebSocket соединения

Убедитесь, что пользователь подключён к комнате текстового канала
